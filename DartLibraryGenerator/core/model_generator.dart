...
FutureOr<LocalModel> generateModel(String libraryPath, ApiModel model, List<LocalEntityDeclaration> modelDeclarations) {
  ...
  File exportsFile = File(path.join(libraryPath, LIBRARY_SOURCE_FOLDER_NAME, MODEL_FOLDER_NAME, "$MODEL_FOLDER_NAME.dart"));
  ...
  for (ApiEntity entity in model.entities) {
    ...
    File outputFile = File(path.join(libraryPath, LIBRARY_SOURCE_FOLDER_NAME, MODEL_FOLDER_NAME, declaration.localPath));
...
Library _generateExportsFile(List<LocalEntityDeclaration> modelDeclarations) {
  return Library((library) => library.directives.addAll(
    List.generate(modelDeclarations.length, (i) =>
      Directive.export(modelDeclarations[i].localPath.replaceAll(r'\', r'\\'))
    )));
}
  ...
  Library entityImplementation = new Library((library) => library
    ...
    ..body.add(
      new Class((entityClass) {
        ...
        entityClass.annotations.add(new InvokeExpression.newOf(refer(JSON_SERIALIZABLE_ANNOTATION_NAME, JSON_ANNOTATIONS_URL), []));
        ...
        for (ApiEntityProperty property in entity.properties) {
          entityClass.fields.add(new Field((entityField) {
            ...
            entityField.annotations.add(InvokeExpression.newOf(
              refer(JSON_KEY_ANNOTATION_NAME, JSON_ANNOTATIONS_URL), 
              [], 
              { "name": new CodeExpression(new Code('"${property.jsonName}"')) }));

            if (!property.type.nullable && declaration.kind != ApiEntityKind.Parameters) {
